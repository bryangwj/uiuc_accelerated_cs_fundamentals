...
void downHeap(Node *n) {
  ...
  // Initialise the left and right values as the largest possible int
  int leftValue = std::numeric_limits<int>::max(); 
  int rightValue = std::numeric_limits<int>::max();

  int minChildValue, currentValue;
  Node * minChild;

  // Check that left and right pointers of node n are not null, and then assign the values
  if (n->left){
  ...
  if (!isLeaf(n)){
    if (n->value > leftValue || n->value > rightValue){
      minChildValue = std::min(leftValue, rightValue);
      currentValue = n->value;

      // Swap with left child
      if (minChildValue == leftValue){
        minChild = n->left;
        n->value = minChildValue;
        minChild->value = currentValue;
        downHeap(minChild);
      }

      // Swap with right child
      else if (minChildValue == rightValue){
        minChild = n->right;
        n->value = minChildValue;
        minChild->value = currentValue;
        downHeap(minChild);
