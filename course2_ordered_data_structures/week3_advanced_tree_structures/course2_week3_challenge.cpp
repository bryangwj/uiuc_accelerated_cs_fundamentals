...
int computeNodeHeight(Node* n) {
  ...
  if (n->left) {
    leftHeight = computeNodeHeight(n->left) + 1;
  } 
  // Else when left child is null
  else {
    leftHeight = 0;
  }
  ...
  if (n->right) {
    rightHeight = computeNodeHeight(n->right) + 1;
  } 
  
  // Else when right child is null
  else {
    rightHeight = 0;
  }

  int nodeHeight = std::max(leftHeight, rightHeight);
  n->height = nodeHeight;
  return nodeHeight;
}
