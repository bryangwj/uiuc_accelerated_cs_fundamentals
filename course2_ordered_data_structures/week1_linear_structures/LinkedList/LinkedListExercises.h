...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // Allocate a new node with the new data
  ...
  if (!head_){
    ...
    // Loops through the entire list until cur == nullptr
    while (cur){

      T dataToCompare = cur->data;

      if (dataToCompare > newData){
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // If only left list is empty
  if (left.empty() && !right.empty()){
    return right;
  }

  // If only right list is empty
  if(!left.empty() && right.empty()){
    return left;
  }

  // If both lists are not empty, loop through and pop elements until one of them becomes empty
  ...
  // If the left list is empty, add all elements in the right list into the merged list
  if (left.empty()){
    while(!right.empty()){
      T rightFront = right.front();
      right.popFront();
  ...
  // For the case when the right list is empty
  else{
    while(!left.empty()){
      T leftFront = left.front();
      left.popFront();
