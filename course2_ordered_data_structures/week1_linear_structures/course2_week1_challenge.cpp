...
std::queue<int> reverse_queue(std::queue<int> q) {
  ...
  std::stack<int> intermediate_stack;
  while(!q.empty()){
    int front_int = q.front();
    intermediate_stack.push(front_int);
    q.pop();
  }

  while(!intermediate_stack.empty()){
    int top_int = intermediate_stack.top();
    reversed_q.push(top_int);
